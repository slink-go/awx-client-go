...
type JobTemplateSurveySpecResource struct {
	Resource
}

func NewJobTemplateSurveySpecResource(connection *Awx, path string) *JobTemplateSurveySpecResource {
	resource := new(JobTemplateSurveySpecResource)
	resource.connection = connection
	resource.templatePath = path
	return resource
}

func (r *JobTemplateSurveySpecResource) Get(id int) *JobTemplateSurveySpecGetRequest {
	request := new(JobTemplateSurveySpecGetRequest)
	request.resource = &r.Resource
	r.path = fmt.Sprintf(r.templatePath, id)
	return request
}

type JobTemplateSurveySpecGetRequest struct {
	Request
}

func (r *JobTemplateSurveySpecGetRequest) Send() (response *TemplateSurveySpecGetResponse, err error) {
